...
/** Generates a report through the official Google Gemini API. */
export const generateReportWithGoogle = async (chart: BaziChart): Promise<Report> => {
  const ai = new GoogleGenAI({ apiKey: GEMINI_API_KEY });
  // Guards against a leftover Kie model id when switching provider.
  const model = GEMINI_MODEL && !GEMINI_MODEL.endsWith('-openai') ? GEMINI_MODEL : DEFAULT_MODEL;
