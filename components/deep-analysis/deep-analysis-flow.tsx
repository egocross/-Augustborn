...
export function DeepAnalysisFlow({ birthInput, freeReport, price }: { birthInput: BirthInput; freeReport: Report; price: string }) {
  const [state, dispatch] = useReducer(deepFlowReducer, undefined, () => {
    if (typeof window !== 'undefined') {
      const restored = loadDeepSession(window.sessionStorage);
      if (restored?.birthInput && restored.freeReport) return restored;
    }
    const id = typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : `session-${Date.now()}`;
    return createInitialDeepState(id, { birthInput, freeReport });
  });

  useEffect(() => { saveDeepSession(state, window.sessionStorage); }, [state]);
  ...
  async function generateReport() {
    try {
      ...
      try {
        ...
        dispatch({ type: 'generationSucceeded', report: await consumeDeepReportStream(response.body) });
  ...
  if (state.step === 'custom-question' || state.step === 'custom-loading') return <section className="deep-panel question-panel">
    ...
    <textarea aria-label="我的问题" disabled={state.step === 'custom-loading'} maxLength={1000} onChange={(event) => dispatch({ type: 'setCustomQuestion', value: event.target.value })} rows={5} value={state.customQuestion} />
  ...
  if (state.step === 'payment') return <section className="deep-panel payment-panel"><p className="eyebrow">专项深度分析</p><h2>你的深度分析已经准备好</h2><p className="deep-lead">系统会综合出生信息、基础报告、校准问题与补充信息，生成更具体的专项报告。</p><p className="price-label">{price}</p>{state.errorCode ? <p className="form-error" role="alert">上次操作未完成，你的答案已保留，可以重试。</p> : null}<button className="primary-button" onClick={generateReport} type="button">生成我的深度报告</button><p className="mock-note">当前为 Mock Payment，不会实际扣款。</p></section>;
  if (state.step === 'generating') return <section aria-live="polite" className="deep-panel deep-generating" role="status"><span aria-hidden="true" className="generating-orb" /><h2>正在结合你的现实情况</h2><p>正在分析最值得优先探索的方向…</p></section>;
  if (state.step === 'report' && state.report) return <section className="deep-panel"><p className="eyebrow">专项报告</p><h2>{state.report.title}</h2><p>{state.report.summary}</p></section>;
  return null;
}
