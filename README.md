...
正式生成工作方向报告时，后台先通过 Gemini 的 Google Search 工具检索招聘实例，再把带引用的证据交给报告模型。目标为 3–5 个真实职位名称，每项提供搜索关键词、适配理由、待核对门槛、验证动作、招聘来源链接和查询日期。行业与城市使用下述独立的公开资料检索；共事和自定义方向不增加搜索步骤。
...
- 行业先展示 2–3 组细分标签与交集，再给出本次检索支持的 3–5 个实例；资料不足不凑数。
- 城市按个人探索优先级分三梯队，并非一二三线或全国城市实力排名。目标每梯队 1–3 个有来源的候选；允许空缺。选择“只考虑当前城市”时，后端会过滤其他城市。
- 行业与城市每次正式生成增加一次 Gemini Google Search 请求（55 秒限时）；搜索使用固定问卷标签及用户声明的城市名，不发送出生数据、基础报告或自由补充。报告模型仍结合完整基础报告、现实答案与补充信息判断。
- 检索优先近期政府/统计/产业部门、信通院与 CNNIC 资料。只有搜索返回的引用与对应摘要能进入证据列表；链接、查询时间由服务端提供。摘要是 AI 整理，并非原文；查询时间不等于发布日期，规划不代表已落地。没有可靠来源时保留探索框架，并明确不展示具体实例和最新趋势。
- 共事方向给出互补能力标签、共同价值链及 2–4 个分工场景，包含双方职责、共同决策、试合作与风险信号，不对真实他人作出生信息或人格推断。
- 提示词分别在 `lib/deep-analysis/prompts/{industry,city,collaboration}.ts`，检索提示词在 `market-research.ts`；服务在 `lib/deep-analysis/research/market.ts`。沿用现有 Gemini 环境变量，无新增密钥或数据库迁移。
- 新结构用于新生成的报告，旧报告保持兼容。`REPORT_PROVIDER=sample` 仍不联网，固定示例不伪造来源。
